...
export function toReadableLocalPath(p: string | null | undefined): string {
  try {
    if (!p || typeof p !== 'string') return '';
    const abs = path.resolve(p);
    if (abs.indexOf('/TemporaryItems/') === -1) return path.normalize(abs);
    try {
      if (!fs.existsSync(COPY_DIR)) fs.mkdirSync(COPY_DIR, { recursive: true });
    } catch (e) { }
...
export async function resolveSafeLocalPath(p: string | null | undefined): Promise<string> {
  try {
...
export async function normalizePaths(obj: { videoPath?: string; audioPath?: string } | null | undefined): Promise<{ videoPath?: string; audioPath?: string } | null | undefined> {
  if (!obj) return obj;
...
export function normalizeOutputDir(p: string | null | undefined): string {
  try {
    if (!p || typeof p !== 'string') return '';
    const abs = path.resolve(p);
...
export function guessMime(p: string | null | undefined): string {
